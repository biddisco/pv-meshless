...
//----------------------------------------------------------------------------
vtkstd::string FileSeriesFinder::GenerateGlobString(int index, stringlist &patterns, bool regexmode)
{
...
vtkstd::string FileSeriesFinder::GenerateFileName(int T, int B, int V)
{
  ...
  for (unsigned int i=0; i<regexmatches.size(); i++) {
    if      (i==TimeIndex) result += Tstrings[T];
    else if (i==BlockIndex) result += Bstrings[B];
    else if (i==VarIndex) result += Vstrings[V];
