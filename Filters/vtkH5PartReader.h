...
public:
  ...
  // coordinate/vector array, they can also be added to the field data for
  // visualization. Default is off as it uses extra memory.
  ...
  // Description:
  // Normally, a request for data at time t=x, where x is either before the
  // start of time for the data, or after the end, will result in the first or
  // last timestep of data to be retrieved (time is clamped to max/min values).
  ...
  // the (update) information part of the pipeline has been updated, and before
  // the (update) data part is updated.
  ...
  const char *GetPointArrayStatusArrayName(int index) {
    return GetPointArrayName(index);
  }
  int GetPointArrayStatusArrayStatus(const char *name) {
    return GetPointArrayStatus(name);
  }
...
protected:
  ...
  int PartitionByExtentsRandomized(vtkIdType N,
                                   std::vector<vtkIdType> &startend);

  // Use the BoundingBoxes read to partition data for reading
  int PartitionByBoundingBoxes(std::vector<vtkIdType> &minIds,
  ...
  // Under normal circumstances, when loading data and animating though
  // timesteps one does not want to close the file between steps (calls to
  // ExecuteInfo/Data) but subclasses (e.g. dsm) do need to close the file for
  // real. We therefore call CloseFileIntermediate when we can leave it open and
  // subclasses can decide to act on it, or do nothing. By default, do nothing.
