...
#define ZOLTAN_PRINT_ERROR(b, c) {printf("%s %s\n", b, c);exit(-1);}
...
double Zoltan_HSFC_InvHilbert1d (double *coord)
   {
   char *yo = "Zoltan_HSFC_InvHilbert1d";

   /* sanity check for input arguments */
   if (coord[0] < 0.0)
      ZOLTAN_PRINT_ERROR (yo, "Spatial Coordinates out of range.");
...
double Zoltan_HSFC_InvHilbert2d (double *coord)
   {
...
double Zoltan_HSFC_InvHilbert3d (double *coord)
   {
...
void Zoltan_HSFC_Hilbert1d (double *coord, double key)
   {
   char *yo = "Zoltan_HSFC_Hilbert1d";

   /* sanity check for input argument */
   if ((key < 0.0) || (key > 1.0))
      ZOLTAN_PRINT_ERROR (yo, "Hilbert coordinate out of range.");
...
void Zoltan_HSFC_Hilbert2d (double *coord, double key)
   {
...
void Zoltan_HSFC_Hilbert3d (double *coord, double key)
   {
   static const unsigned int *d[] =
...
preserve the maximum precision.  This requires changing the function definition:
   double Zoltan_HSFC_InvHilbert2d (double *coord)
to
   int *Zoltan_HSFC_InvHilbert2d (double *coord).
...
current double.  This requires changing the function definition:
   void Zoltan_HSFC_Hilbert2d (double *coord, double key)
to
   void Zoltan_HSFC_Hilbert2d (double *coord, int* ikey)
Change the declaration
