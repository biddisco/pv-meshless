...
// mpiexec --localonly -n 4 TestH5PartParallelWriter -I -D D:\ -F sph-test.h5 -R
// -X -N 1000000
...
std::string usage =
    ...
    "\t\n"
    "\tWIN32 mpi example   : mpiexec -n 4 -localonly "
    "TestH5PartParallelWriter.exe -D D:\\ -F sph-test.h5 -X\n"
    "\tLinux mpich example : "
    "/project/csvis/biddisco/build/mpich2-1.0.7/bin/mpiexec -n 2 "
    "./TestH5PartParallelWriter -D /project/csvis/hdf-scratch -C -N 1000000 "
    "-X\n"
    "\tHorus Slurm example : mpirun -prot -srun mpirun -e DISPLAY=horus11:0 -e "
    "MPI_IB_CARD_ORDER=0:0 -e MPI_IB_MTU=1024 -e "
    "MPI_IC_ORDER=ibv:vapi:udapl:itapi:TCP -srun -N 10 -n 20 "
    "bin/TestH5PartParallelWriter -D /project/csvis/hdf-scratch -C -N 100000 "
    "-R -I -X\n";
...
int main(int argc, char *argv[]) {
  ...
  //--------------------------------------------------------------
  if (test.myRank == 0 &&
      vtksys::SystemTools::FileExists(test.fullName.c_str())) {
    DisplayParameter<std::string>("Delete file", "", &test.fullName, 1,
                                  (test.myRank == 0) ? 0 : -1);
  ...
  double MBPerParticle =
      (sizeof(vtkIdType) + sizeof(double) + sizeof(float) + 3 * sizeof(float)) /
      (1024.0 * 1024.0);
  ...
  double totalmemorywriteMB =
      numPoints * MBPerParticle * test.iterations * test.numProcs;
  double totalmemorywriteGB = totalmemorywriteMB / 1024.0;

  DisplayParameter<vtkIdType>("Particles requested", "", &numPoints, 1,
                              (test.myRank == 0) ? 0 : -1);
  DisplayParameter<double>("Memory per process ", "(MB)", &memoryusedMB, 1,
                           (test.myRank == 0) ? 0 : -1);
  DisplayParameter<double>("Memory per iteration ", "(GB)",
                           &totalmemoryperIterationGB, 1,
                           (test.myRank == 0) ? 0 : -1);
  DisplayParameter<double>("Memory total to write ", "(GB)",
                           &totalmemorywriteGB, 1, (test.myRank == 0) ? 0 : -1);

  //
  vtkSmartPointer<vtkPolyData> Sprites = vtkSmartPointer<vtkPolyData>::New();
  vtkSmartPointer<vtkPoints> points = vtkSmartPointer<vtkPoints>::New();
  vtkSmartPointer<vtkCellArray> verts = vtkSmartPointer<vtkCellArray>::New();
  vtkSmartPointer<vtkDoubleArray> sizes =
  ...
  // Create writer
  vtkSmartPointer<vtkH5PartWriter> writer =
      vtkSmartPointer<vtkH5PartWriter>::New();
  ...
  */
  ...
  test.controller->Barrier();
  DisplayParameter<vtkIdType>("Particles generated", "", &numPoints, 1,
                              (test.myRank == 0) ? 0 : -1);
  //
  ...
  DisplayParameter<double>("File Written in", "", &elapsed, 1,
                           (test.myRank == 0) ? 0 : -1);
  DisplayParameter<double>("IO-Speed ", "(MB/s)", &IOSpeed, 1,
                           (test.myRank == 0) ? 0 : -1);
  ...
  tempdata << test.testName << "," << test.numNodes << ","
           << test.processesPerNode << "," << memoryusedMB << ","
           << totalmemoryperIterationGB << "," << totalmemorywriteMB << ","
           << totalmemorywriteGB << "," << numPoints << "," << IOSpeed;
  std::string infostring = tempdata.str();
  DisplayParameter<std::string>("CSVData ", "", &infostring, 1,
                                (test.myRank == 0) ? 0 : -1);

  //    std::cout << "Process Id : " << test.myRank << " File Written in " <<
  //    elapsed << " seconds" << std::endl; std::cout << "Process Id : " <<
  //    test.myRank << " IO-Speed " << MBytes/timer->GetElapsedTime() << " MB/s"
  //    << std::endl;
  //
  std::cout << std::endl;
  std::cout << " * * * * * * * * * * * * * * * * * * * * * * * * * "
            << std::endl;
  std::cout << "Process Id : " << test.myRank << " Expected "
            << static_cast<vtkTypeInt64>(numPoints * test.numProcs)
            << std::endl;
  ...
  // All ranks must participate in the collective MPI-IO read.
  {
    ...
    if (test.myRank == 0) {
      ...
      std::cout << "Process Id : " << test.myRank << " Read : " << ReadPoints
                << std::endl;
      std::cout << " * * * * * * * * * * * * * * * * * * * * * * * * * "
                << std::endl;
      //
      ...
      vtkIdTypeArray *pointIds = vtkIdTypeArray::SafeDownCast(
          reader->GetOutput()->GetPointData()->GetArray("PointIds"));
      bool IdsGood = true;
      ...
      if (IdsGood && ReadPoints == numPoints * test.numProcs) {
        ...
        if (test.myRank == 0) {
          unsigned long size =
              vtksys::SystemTools::FileLength((char *)(test.fullName.c_str()));
          double filesize = size / (1024.0 * 1024.0);
          std::cout << "Process Id : " << test.myRank << " Total IO/Disk-Speed "
                    << filesize / elapsed << " MB/s" << std::endl;
        }
      } else {
        ok = false;
        std::cout << " " << std::endl;
      ...
      if (test.doRender) {
        std::cout << "Process Id : " << test.myRank << " Rendering"
                  << std::endl;

        // Generate vertices from the points
        vtkSmartPointer<vtkMaskPoints> verts =
        ...
        polys->GetPointData()->SetScalars(
            polys->GetPointData()->GetArray("Elevation"));
        std::cout << "Process Id : " << test.myRank
                  << " Created vertices : " << polys->GetNumberOfPoints()
                  << std::endl;
        //
        ...
        std::cout << "Process Id : " << test.myRank << " About to Render"
                  << std::endl;
        renWindow->Render();
